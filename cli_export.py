...
def export_all_habits_to_csv(db_name: str, base_dir: str = "CSV Export"):
    ...
    filename = os.path.join(
        base_dir, f"habits_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
    ...
    try:
        ...
        show_colored_message(
            f"Successfully exported {len(habits)} habits to {filename}",
            color=Fore.GREEN,
        )
...
def export_all_completions_to_csv(db_name: str, base_dir: str = "CSV Export"):
    ...
    filename = os.path.join(
        base_dir,
        f"all_completions_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
    )
    ...
    try:
        ...
        show_colored_message(
            f"Successfully exported {total_completions} completions to {filename}",
            color=Fore.GREEN,
        )
    except Exception as e:
...
def export_habit_completions_to_csv(db_name: str, base_dir: str = "CSV Export"):
    ...
    selected_habit = _handle_habit_selection(
        habits, "Select a habit to export completions:", "No habits found to export."
    )
    ...
    if not completions:
        show_colored_message(
            f"No completions found for '{selected_habit.name}'.", color=Fore.YELLOW
        )
        press_enter_to_continue()
        return
    ...
    filename = os.path.join(
        base_dir,
        f"completions_{selected_habit.name.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
    )
    ...
    try:
        with open(filename, "w", newline="", encoding="utf-8") as csvfile:
            ...
            for completion in completions:
                ...
                if selected_habit.frequency == "weekly":
                    ...
                    week_start = completion_date - timedelta(
                        days=(
                            completion_date.weekday() + 1
                            if completion_date.weekday() < 6
                            else 0
                        )
                    )
        ...
        show_colored_message(
            f"Successfully exported {len(completions)} completions for '{selected_habit.name}' to {filename}",
            color=Fore.GREEN,
        )
    except Exception as e:
        show_colored_message(f"Error exporting completions: {str(e)}", color=Fore.RED)
