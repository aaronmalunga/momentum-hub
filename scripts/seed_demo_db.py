...
def seed_data(conn):
    ...
    habits = [
        ("Read 20 pages", "daily", "Demo habit: reading", "08:00", "20:00", 5, now, now, 1, None),
        ("Stretch 10 min", "daily", "Morning stretch", "07:00", "19:00", 3, now, now, 1, None)
    ]
    cur.executemany("""
...
def show_preview(conn, db_path: Path):
    ...
    print("📘 Habits:")
    print("-" * 70)
    for row in cur.execute("SELECT id, name, frequency, streak, is_active FROM habits;"):
