...
from cli_utils import _validate_time_format, _handle_habit_selection
...
def create_new_habit(db_name: str):
    ...
    while True:
        habit_name = questionary.text("Enter habit name (or type 'cancel' to go back):").ask()
    ...
    frequency = questionary.select(
        f"How often should '{habit_name}' be done?",
...
def mark_habit_completed(db_name: str):
    ...
    selected_habit = _handle_habit_selection(habits, "Select a habit to mark as completed:", "No active habits found. Let's create one!")
    ...
    try:
        ...
        show_colored_message(f"'{habit.name}' marked as completed! Current streak: {updated_habit.streak}", color=Fore.GREEN, style=Style.BRIGHT)
    except ValueError as e:
        show_colored_message(f"Cannot mark '{habit.name}' as completed: {str(e)}", color=Fore.RED, style=Style.BRIGHT)
        press_enter_to_continue()
        return
    show_colored_message(encouragements.get_completion_encouragement(), color=Fore.CYAN, style=Style.BRIGHT)
...
def delete_habit(db_name: str):
    ...
    habit_to_delete = _handle_habit_selection(habits, "Select a habit to delete (or Cancel):", "No active habits found to delete.")
    if not habit_to_delete:
        return  # _handle_habit_selection already shows cancellation message and waits for input
    confirm = questionary.confirm(f"Are you sure you want to delete '{habit_to_delete.name}'?").ask()
    if not confirm:
        show_colored_message("Deletion cancelled.", color=Fore.YELLOW)
        press_enter_to_continue()
        return
    db.delete_habit(habit_to_delete.id, db_name)
    show_colored_message(f"Habit '{habit_to_delete.name}' deleted (deactivated) successfully!", color=Fore.GREEN)
    press_enter_to_continue()
...
def reactivate_habit(db_name: str):
    ...
    if not deleted_habits:
        ...
        return
    # Use interactive menu for habit selection, with Cancel option
    choices = [f"{habit.id}. {habit.name} ({habit.frequency})" for habit in deleted_habits]
    choices.append("Cancel")
    answer = questionary.select("Select a habit to reactivate (or Cancel):", choices=choices).ask()
    ...
    try:
        ...
        for habit in deleted_habits:
            if habit.id == habit_id:
                ...
                show_colored_message(f"Habit '{habit.name}' reactivated successfully!", color=Fore.GREEN)
                press_enter_to_continue()
                return
    except Exception:
...
def update_habit(db_name: str):
    ...
    habits = db.get_all_habits(active_only=True, db_name=db_name)
    if not habits:
        show_colored_message("No active habits found. Let's create one!", color=Fore.RED)
        press_enter_to_continue()
        return

    # Use interactive menu for habit selection
    habit_to_update = _handle_habit_selection(habits, "Select a habit to update (or Cancel):", "No active habits found to update.")
    if not habit_to_update:
        show_colored_message("Update cancelled.", color=Fore.YELLOW)
        press_enter_to_continue()
        return

    show_colored_message(f"\n--- Updating Habit ID: {habit_to_update.id} - '{habit_to_update.name}' ---", color=Fore.CYAN, style=Style.BRIGHT)
    if habit_to_update.reactivated_at:
        show_colored_message(f"Reactivated At: {habit_to_update.reactivated_at.strftime('%Y-%m-%d %H:%M')}", color=Fore.CYAN)
    ...
    new_evening_reminder_time = questionary.text(
        ...
        default=habit_to_update.evening_reminder_time if habit_to_update.evening_reminder_time else "",
        validate=_validate_time_format
    ).ask()
    ...
    show_colored_message(f"Habit ID {habit_to_update.id} updated successfully!", color=Fore.GREEN, style=Style.BRIGHT)
    press_enter_to_continue()
