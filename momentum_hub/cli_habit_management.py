...
from .encouragements import get_completion_encouragement, get_streak_encouragement
...
def create_new_habit(db_name: str):
    ...
    # Ensure schema exists for tests or first-run scenarios
    db.init_db(db_name)
...
def mark_habit_completed(db_name: str):
    ...
    try:
        ...
        current_streak = (
            updated_habit.streak if updated_habit is not None else habit.streak
        )
        show_colored_message(
            f"'{habit.name}' marked as completed! Current streak: {current_streak}",
            color=Fore.GREEN,
            style=Style.BRIGHT,
        )
...
def update_habit(db_name: str):
    ...
    category_prompt = (
        f"Select new category for '{habit_to_update.name}' "
        f"(current: {current_category.name if current_category else 'None'}):"
    )
    category_choice = questionary.select(
        category_prompt,
