...
class DemoHabit(TypedDict):
    name: str
    frequency: str
    notes: str
    reminder_time: Optional[str]
    evening_reminder_time: Optional[str]
    category: str


class DemoGoal(TypedDict):
    habit_name: str
...
def create_demo_habits(db_name: str) -> None:
    ...
    for habit_data in demo_habits:
        ...
        habit.id = habit_id
        category_name = habit_data.get("category")
        habit.category_id = (
            category_ids.get(category_name) if isinstance(category_name, str) else None
        )
    ...
    for goal_data in demo_goals:
        ...
        habit_for_goal = next(
            (
                h
                for h in db.get_all_habits(db_name=db_name)
                if h.name == goal_data["habit_name"]
            ),
            None,
        )
        if habit_for_goal:
            goal = Goal(
                habit_id=habit_for_goal.id,
    ...
    print(
        (
            f"\n??? Created {len(demo_categories)} demo categories, "
            f"{created_count} demo habits, and {len(demo_goals)} demo goals!"
        )
    )
...
def create_demo_with_history(db_name: str) -> None:
    """
    Creates demo categories, habits, goals and a minimal set of completions
    so the demo database shows example analytics and streaks.

    This is intended for demo mode where a user wants to see the app
    with a small amount of plausible history. It is idempotent when run
    against an empty DB (called by `momentum_main --demo` on first-run).

    Args:
        db_name: The database file name to populate
    """
    # Ensure base demo categories/habits/goals exist
    create_demo_habits(db_name)

    # Add a single completion for each habit (current timestamp) to give
    # the demo some visible streaks/completions. Use the DB helpers so
    # concurrency rules and duplicate checks are respected.
    now = datetime.datetime.now()
    habits = db.get_all_habits(active_only=False, db_name=db_name)
    added = 0
    for habit in habits:
        try:
            db.add_completion(habit.id, now, db_name)
            # Recalculate streak after adding completion
            db.update_streak(habit.id, db_name)
            added += 1
        except Exception:
            # If a completion cannot be added (e.g., duplicate), skip
            continue

    print(
        f"\n✓ Demo history added: {added} completion(s) inserted to show analytics/streaks.\n"
    )
