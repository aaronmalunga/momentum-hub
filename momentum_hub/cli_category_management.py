...
class CategoryManager:
    """Manages category operations for the CLI."""

    def __init__(self, db_name: str):
        self.db_name = db_name

    def get_menu_options(self) -> list[str]:
        """Get category management menu options."""
        return [
    ...
    def get_menu_actions(self) -> dict:
        """Get menu action mappings."""
        return {
            "Create a new category": self.create_category,
            "View all categories": self.view_categories,
            "Update a category": self.update_category,
            "Delete a category": self.delete_category,
            "Assign habit to category": self.assign_habit_to_category,
    ...
    def create_category(self):
        ...
        category_id = db.add_category(category, self.db_name)
        show_colored_message(
    ...
    def view_categories(self):
        ...
        categories = db.get_all_categories(active_only=True, db_name=self.db_name)
        if not categories:
        ...
        for category in categories:
            habits = category.get_habits(self.db_name)
    ...
    def update_category(self):
        ...
        if not categories:
            show_colored_message(
                "No active categories found to update.", color=Fore.RED
            )
            press_enter_to_continue()
            return
        ...
        answer = questionary.select(
            "Select a category to update:", choices=choices
        ).ask()
        ...
        try:
            ...
            category = db.get_category(category_id, self.db_name)
            if not category:
        ...
        new_color = questionary.text(
            "Enter new color (leave blank to keep current):",
            default=category.color or "",
        ).ask()
        ...
        db.update_category(category, self.db_name)
        show_colored_message(
    ...
    def delete_category(self):
        ...
        if not categories:
            show_colored_message(
                "No active categories found to delete.", color=Fore.RED
            )
            press_enter_to_continue()
            return
        ...
        answer = questionary.select(
            "Select a category to delete:", choices=choices
        ).ask()
        if answer == "Cancel" or answer is None:
        ...
        for habit in habits:
            ...
            db.update_habit(habit, self.db_name)

        db.delete_category(category_id, self.db_name)
        show_colored_message(
            f"Category '{category.name}' deleted successfully!", color=Fore.GREEN
        )
        press_enter_to_continue()

    def assign_habit_to_category(self):
        """Handles assigning a habit to a category."""
        show_colored_message(
            "\n--- Assign Habit to Category ---", color=Fore.YELLOW, style=Style.BRIGHT
        )

        habits = db.get_all_habits(active_only=True, db_name=self.db_name)
        if not habits:
        ...
        if answer == "Remove from category":
            ...
            db.update_habit(selected_habit, self.db_name)
            show_colored_message(
                f"Habit '{selected_habit.name}' removed from category!",
                color=Fore.GREEN,
            )
        else:
            try:
...
def manage_categories(db_name: str):
    """Legacy function that creates a CategoryManager instance and calls manage_categories."""
    manager = CategoryManager(db_name)
    manager.manage_categories()


def create_category(db_name: str):
    """Legacy function for creating a category."""
    manager = CategoryManager(db_name)
    manager.create_category()


def view_categories(db_name: str):
    """Legacy function for viewing categories."""
    manager = CategoryManager(db_name)
    manager.view_categories()


def update_category(db_name: str):
    """Legacy function for updating a category."""
    manager = CategoryManager(db_name)
    manager.update_category()


def delete_category(db_name: str):
    """Legacy function for deleting a category."""
    manager = CategoryManager(db_name)
    manager.delete_category()


def assign_habit_to_category(db_name: str):
    """Legacy function for assigning habit to category."""
    manager = CategoryManager(db_name)
    manager.assign_habit_to_category()
