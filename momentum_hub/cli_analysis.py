...
from . import habit_analysis as analysis
...
def get_periodicity_choice() -> Optional[str]:
    """Get user's periodicity selection with validation."""
    choice = questionary.select(
        "Select a periodicity to filter habits:",
        choices=["daily", "weekly", "all", "Cancel"],
    ).ask()
...
def filter_habits_by_periodicity(habits: List[Habit], periodicity: str) -> List[Habit]:
    """Filter habits based on selected periodicity."""
    if periodicity == "all":
        return habits
    return [h for h in habits if h.frequency == periodicity]


def format_streak_color(streak: int) -> str:
    """Format streak with appropriate color."""
    color = Fore.GREEN if streak > 0 else Fore.RED
    return f"{color}{streak}{Style.RESET_ALL}"


def format_completion_rate(rate: float) -> str:
    """Format completion rate with appropriate color."""
    percentage = rate * 100
    if percentage >= 80:
        color = Fore.GREEN
    elif percentage >= 50:
        ...
        color = Fore.RED
    return f"{color}{percentage:.0f}%{Style.RESET_ALL}"


def format_habit_data(habit: Habit, db_name: str) -> dict:
    """Format habit data for display in periodicity analysis."""
    return {
        "id": habit.id,
        "name": habit.name,
        "frequency": habit.frequency,
        "current_streak": format_streak_color(habit.streak),
        "created_at": (
            habit.created_at.strftime("%Y-%m-%d %H:%M") if habit.created_at else "-"
        ),
        "longest_streak": analysis.calculate_longest_streak_for_habit(
            habit.id, db_name
        ),
        "completion_rate": format_completion_rate(
            analysis.calculate_completion_rate_for_habit(habit.id, db_name)
        ),
        "last_completed": (
...
def display_streak_analysis_table(habits: List[Habit], db_name: str):
    """Display the streak analysis table for all habits."""
    table = []
    for habit in habits:
        ...
        current_streak_col = format_streak_color(habit.streak)
        longest_streak_col = format_streak_color(longest_streak)
    ...
    headers = [
        ...
        f"{Fore.CYAN}Periodicity{Style.RESET_ALL}",
        f"{Fore.CYAN}Created At{Style.RESET_ALL}",
        f"{Fore.CYAN}Longest Streak{Style.RESET_ALL}",
        f"{Fore.CYAN}Current Streak{Style.RESET_ALL}",
    ]
    print(tabulate(table, headers=headers, tablefmt="grid", stralign="center"))


def display_periodicity_analysis_table(habits: List[Habit], db_name: str):
    """Display the periodicity analysis table for filtered habits."""
    table = []
    for habit in habits:
        habit_data = format_habit_data(habit, db_name)
        table.append(
            [
                habit_data["id"],
                habit_data["name"],
                ...
                format_streak_color(habit_data["longest_streak"]),
                habit_data["completion_rate"],
                habit_data["last_completed"],
            ]
        )
    ...
    headers = [
        ...
        f"{Fore.CYAN}Created At{Style.RESET_ALL}",
        f"{Fore.CYAN}Longest Streak{Style.RESET_ALL}",
        f"{Fore.CYAN}Completion Rate{Style.RESET_ALL}",
...
def analyze_habits(db_name: str):
    ...
    analysis_actions = {
        ...
        "List all habits with the same periodicity": (
            lambda: analyze_by_periodicity(db_name)
        ),
        "See the longest streak of all habits": lambda: analyze_longest_streak_all(
            db_name
        ),
        "See the longest streak of a specific habit": (
            lambda: analyze_longest_streak_one(db_name)
        ),
        "Show streak history (Calendar View)": (
            lambda: analyze_streak_history_grid(db_name)
        ),
        ...
        "Show completion history for a habit": lambda: analyze_completion_history(
            db_name
        ),
...
def analyze_by_periodicity(db_name: str):
    """Analyze habits grouped by their periodicity (daily/weekly/all)."""
    show_colored_message(
    ...
    periodicity_choice = get_periodicity_choice()
    ...
    filtered_habits = filter_habits_by_periodicity(habits, periodicity_choice)
    if not filtered_habits:
        show_colored_message(f"No {periodicity_choice} habits found.", color=Fore.RED)
    else:
        display_periodicity_analysis_table(filtered_habits, db_name)

    press_enter_to_continue()


def analyze_longest_streak_all(db_name: str):
    """Display longest streaks for all habits."""
    show_colored_message(
    ...
    display_streak_analysis_table(habits, db_name)
    press_enter_to_continue()
...
def analyze_longest_streak_one(db_name: str):
    ...
    selected_habit = _handle_habit_selection(
        ...
        "No active habits found to analyze.",
    )
    if not selected_habit:
        return
    habit_to_analyze = selected_habit
    longest_streak = analysis.calculate_longest_streak_for_habit(
        habit_to_analyze.id, db_name
    )
    last_completed = (
        habit_to_analyze.last_completed.strftime("%Y-%m-%d %H:%M")
        if habit_to_analyze.last_completed
        else "-"
    )
    created_at_str = (
        habit_to_analyze.created_at.strftime("%Y-%m-%d %H:%M")
        if habit_to_analyze.created_at
        else "-"
    )
    if habit_to_analyze.reactivated_at:
        show_colored_message(
            f"Reactivated At: {habit_to_analyze.reactivated_at.strftime('%Y-%m-%d %H:%M')}",
            color=Fore.CYAN,
        )
    current_streak_col = (
        f"{Fore.GREEN}{habit_to_analyze.streak}{Style.RESET_ALL}"
        if habit_to_analyze.streak > 0
        else f"{Fore.RED}{habit_to_analyze.streak}{Style.RESET_ALL}"
    )
    longest_streak_col = (
        f"{Fore.GREEN}{longest_streak}{Style.RESET_ALL}"
        if longest_streak > 0
        else f"{Fore.RED}{longest_streak}{Style.RESET_ALL}"
    )
    table = [
        [
            habit_to_analyze.id,
            habit_to_analyze.name,
            habit_to_analyze.frequency,
            created_at_str,
            longest_streak_col,
            current_streak_col,
            last_completed,
    ...
    headers = [
        ...
        f"{Fore.CYAN}Periodicity{Style.RESET_ALL}",
        f"{Fore.CYAN}Created At{Style.RESET_ALL}",
        f"{Fore.CYAN}Longest{Style.RESET_ALL}",
...
def analyze_streak_history_grid(db_name: str):
    ...
    selected_habit = _handle_habit_selection(
        habits,
        "Select a habit to view its streak history:",
        "No active habits found to analyze.",
    )
    if not selected_habit:
        return
    ...
    if habit.reactivated_at:
        show_colored_message(
            f"Reactivated At: {habit.reactivated_at.strftime('%Y-%m-%d %H:%M')}",
            color=Fore.CYAN,
        )
    ...
    if habit.frequency == "daily":
        ...
        if total_completions > 0:
            ...
            show_colored_message(
                (
                    ...
                    f"({month_completions}/{days_in_month} days)"
                ),
                color=Fore.MAGENTA,
            )
    elif habit.frequency == "weekly":
        # Calculate the first week (Sunday) containing the creation date
        creation_week_start = habit_created_date - timedelta(
            days=(habit_created_date.weekday() + 1) % 7
        )
        today = date.today()
        ...
        for i in range(weeks_to_show):
            ...
            if this_week_start > current_week_start:
                ...
                week_row = (
                    week_label + f"{Fore.LIGHTBLACK_EX}  -  {Style.RESET_ALL}" * 7
                )
            else:
                # Check if any completion exists in this week
                week_completed = any(
                    (c >= this_week_start and c <= this_week_end)
        ...
        if weeks_to_show > 0:
            ...
            show_colored_message(
                (
                    f"Completion rate last {weeks_to_show} weeks: "
                    f"{weekly_completion_rate*100:.1f}% "
                    f"({completed_weeks}/{weeks_to_show} weeks)"
                ),
                color=Fore.MAGENTA,
            )
    else:
        show_colored_message(
            "Unsupported periodicity for calendar view.", color=Fore.RED
        )
...
def analyze_best_worst_habit(db_name: str):
    ...
    try:
        ...
        if best_habit and worst_habit:
            table = [
                [
                    f"{Fore.GREEN}Best{Style.RESET_ALL}",
                    f"{Fore.GREEN}{best_habit.name}{Style.RESET_ALL}",
                    f"{Fore.GREEN}{best_habit.streak}{Style.RESET_ALL}",
                ],
                [
                    f"{Fore.RED}Worst{Style.RESET_ALL}",
                    f"{Fore.RED}{worst_habit.name}{Style.RESET_ALL}",
                    f"{Fore.RED}{worst_habit.streak}{Style.RESET_ALL}",
                ],
            ]
            headers = [
            ...
            show_colored_message(
                "No streaks found for any active habits yet. Keep tracking!",
                color=Fore.RED,
            )
...
def analyze_completion_history(db_name: str):
    ...
    if not completion_history:
        ...
        table = [
            [i + 1, dt.strftime("%Y-%m-%d %H:%M")]
            for i, dt in enumerate(completion_history)
        ]
        headers = [
            f"{Fore.CYAN}# {Style.RESET_ALL}",
            f"{Fore.CYAN}Completion Date/Time{Style.RESET_ALL}",
        ]
        print(tabulate(table, headers=headers, tablefmt="grid", stralign="center"))
    press_enter_to_continue()
