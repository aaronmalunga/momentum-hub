...
def create_category(db_name: str):
    ...
    while True:
        name = questionary.text("Enter category name (or type 'cancel' to go back):").ask()
    ...
    color = questionary.text("Enter category color (hex code, optional, e.g., #FF5733):").ask()
    if color is None:
        show_colored_message("Category creation cancelled.", color=Fore.YELLOW)
        press_enter_to_continue()
        return
...
def update_category(db_name: str):
    ...
    new_color = questionary.text(
        f"Enter new color (leave blank to keep current):",
    ...
    show_colored_message(f"Category '{category.name}' updated successfully!", color=Fore.GREEN, style=Style.BRIGHT)
    press_enter_to_continue()
...
def delete_category(db_name: str):
    ...
    confirm = questionary.confirm(f"Are you sure you want to delete category '{category.name}'?").ask()
    if not confirm:
        show_colored_message("Deletion cancelled.", color=Fore.YELLOW)
        press_enter_to_continue()
        return
    ...
    show_colored_message(f"Category '{category.name}' deleted successfully!", color=Fore.GREEN)
    press_enter_to_continue()
...
def assign_habit_to_category(db_name: str):
    ...
    selected_habit = _handle_habit_selection(habits, "Select a habit to assign to a category:", "No active habits found.")
    if not selected_habit:
        return

    categories = db.get_all_categories(active_only=True, db_name=db_name)
    if not categories:
        show_colored_message("No active categories found. Create a category first!", color=Fore.RED)
        press_enter_to_continue()
        return
    ...
    answer = questionary.select(f"Select a category for habit '{selected_habit.name}':", choices=choices).ask()
    if answer == "Cancel" or answer is None:
    ...
    if answer == "Remove from category":
        ...
        show_colored_message(f"Habit '{selected_habit.name}' removed from category!", color=Fore.GREEN)
    else:
        try:
            category_id = int(answer.split(".")[0])
            ...
            show_colored_message(f"Habit '{selected_habit.name}' assigned to category '{category_name}'!", color=Fore.GREEN)
        except ValueError:
            show_colored_message("Invalid selection.", color=Fore.RED)
