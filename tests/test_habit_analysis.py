...
class TestAnalysisFeatures:
    ...
    def test_calculate_longest_streak_change_beddings(self):
        ...
        change_beddings_habit = next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Change beddings")
        streak = habit_analysis.calculate_longest_streak_for_habit(change_beddings_habit.id, self.test_db_name)
        assert streak == 7    
    
    def test_calculate_completion_rate_change_beddings(self):
        """ Test completion rate for 'change beddings' (2/4 weeks completed)."""
        change_beddings_habit = next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Change beddings")
        rate = habit_analysis.calculate_completion_rate_for_habit(change_beddings_habit.id, self.test_db_name)
        assert rate == 0.5 # Completion rate should be 50% (2 out of 4 weeks completed).

    def test_calculate_longest_streak_code_daily(self):
        """Test longest streak for 'Code' (perfect 28-day streak)."""
        code_daily_habit =  next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Code")
        streak = habit_analysis.calculate_longest_streak_for_habit(code_daily_habit.id, self.test_db_name)
        assert streak == 28
    ...
    def test_calculate_completion_rate_code_daily(self):
        ...
        code_daily_habit = next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Code")
        rate = habit_analysis.calculate_completion_rate_for_habit(code_daily_habit.id, self.test_db_name)
        assert rate == 1.0

    def test_get_missed_days_code_daily(self):
        """ Test missed days for 'Code' (should return 0 missed days)."""
        code_daily_habit = next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Code")
        missed_days = habit_analysis.get_missed_days_for_habit(code_daily_habit.id, self.test_db_name)
    ...
    def test_calculate_longest_streak_study(self):
        ...
        study_habit = next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Study")
        streak = habit_analysis.calculate_longest_streak_for_habit(study_habit.id, self.test_db_name)
        assert streak == 23

    def test_calculate_completion_rate_study(self):
        """ Test completion rate for 'Study' (25/28 days completed)."""
        study_habit = next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Study")
        rate = habit_analysis.calculate_completion_rate_for_habit(study_habit.id, self.test_db_name)
    ...
    def test_get_missed_days_study(self):
        ...
        study_habit = next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Study")
        missed_days = habit_analysis.get_missed_days_for_habit(study_habit.id, self.test_db_name)
    ...
    def test_calculate_longest_streak_meditate(self):
        ...
        meditate_habit = next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Meditate")
        streak = habit_analysis.calculate_longest_streak_for_habit(meditate_habit.id, self.test_db_name)
        assert streak == 15

    def test_calculate_longest_streak_blog(self):
        """ Test longest streak for 'Blog' (perfect weekly streak)."""
        blog_habit = next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Blog")
        streak = habit_analysis.calculate_longest_streak_for_habit(blog_habit.id, self.test_db_name)
    ...
    def test_calculate_completion_rate_blog(self):
        ...
        blog_habit = next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Blog")
        rate = habit_analysis.calculate_completion_rate_for_habit(blog_habit.id, self.test_db_name)
    ...
    def test_get_missed_days_weekly_habit_blog(self):
        ...
        blog_habit = next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Blog")
        missed_days = habit_analysis.get_missed_days_for_habit(blog_habit.id, self.test_db_name)
    ...
    def test_calculate_goal_progress_daily(self):
        ...
        progress = habit_analysis.calculate_goal_progress(code_habit.id, self.test_db_name)
    ...
    def test_calculate_goal_progress_weekly(self):
        ...
        blog_habit = next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Blog")
        progress = habit_analysis.calculate_goal_progress(blog_habit.id, self.test_db_name)
    ...
    def test_get_completion_history_sorted(self):
        ...
        code_habit = next(h for h in db.get_all_habits(db_name=self.test_db_name) if h.name == "Code")
        history = habit_analysis.get_completion_history(code_habit.id, self.test_db_name)
