...
from cli_category_management import (
    manage_categories, create_category, view_categories,
    update_category, delete_category, assign_habit_to_category
)
...
class TestCreateCategory:
    def test_create_category_success(self, tmp_db_path):
        with patch('questionary.text') as mock_text, \
            ...
            mock_text.side_effect = lambda *args, **kwargs: MagicMock(ask=MagicMock(return_value=["Test Category", "Test Description", "#FF5733"][mock_text.call_count - 1]))
    ...
    def test_create_category_cancel_name(self, tmp_db_path):
        with patch('questionary.text') as mock_text, \
            ...
            mock_show.assert_called_with("Category creation cancelled.", color='\x1b[33m')
    ...
    def test_create_category_empty_name(self, tmp_db_path):
        with patch('questionary.text') as mock_text, \
            ...
            def side_effect(*args, **kwargs):
                nonlocal call_count
                call_count += 1
                return MagicMock(ask=MagicMock(return_value=["", "Test Category", "Test Description", "#FF5733"][call_count - 1]))
            ...
            mock_show.assert_any_call("Category name cannot be empty.", color='\x1b[31m')
            # And that the success message was called
            mock_show.assert_any_call("Category 'Test Category' created successfully with ID: 1", color='\x1b[32m', style='\x1b[1m')
...
class TestViewCategories:
    def test_view_categories_with_data(self, sample_data):
        ...
        with patch('cli_category_management.show_colored_message'), \
             patch('cli_category_management.press_enter_to_continue'), \
    ...
    def test_view_categories_empty(self, tmp_db_path):
        with patch('cli_category_management.show_colored_message') as mock_show, \
             patch('cli_category_management.press_enter_to_continue'):

            view_categories(tmp_db_path)

            mock_show.assert_called_with("No active categories found.", color='\x1b[31m')
...
class TestUpdateCategory:
    def test_update_category_success(self, sample_data):
        ...
        with patch('questionary.select') as mock_select, \
            ...
            mock_select.return_value = MagicMock(ask=MagicMock(return_value=f"{cat_id1}. Health"))
            call_count = 0
            def side_effect(*args, **kwargs):
                nonlocal call_count
                call_count += 1
                return MagicMock(ask=MagicMock(return_value=["Updated Health", "Updated Description", "#0000FF"][call_count - 1]))
    ...
    def test_update_category_no_categories(self, tmp_db_path):
        with patch('cli_category_management.show_colored_message') as mock_show, \
            ...
            mock_show.assert_called_with("No active categories found to update.", color='\x1b[31m')
    ...
    def test_update_category_invalid_selection(self, sample_data):
        ...
        with patch('questionary.select') as mock_select, \
            ...
            mock_select.return_value = MagicMock(ask=MagicMock(return_value="invalid. selection"))
...
class TestAssignHabitToCategory:
    def test_assign_habit_to_category_success(self, sample_data):
        ...
        with patch('cli_category_management._handle_habit_selection') as mock_handle, \
    ...
    def test_assign_habit_remove_from_category(self, sample_data):
        ...
        with patch('cli_category_management._handle_habit_selection') as mock_handle, \
            ...
            mock_handle.return_value = habit
            mock_select.return_value = MagicMock(ask=MagicMock(return_value="Remove from category"))
    ...
    def test_assign_habit_no_categories(self, tmp_db_path):
        ...
        with patch('cli_category_management._handle_habit_selection') as mock_handle, \
            ...
            mock_show.assert_called_with("No active categories found. Create a category first!", color='\x1b[31m')
...
class TestManageCategories:
    def test_manage_categories_create(self, tmp_db_path):
        with patch('questionary.select') as mock_select, \
            ...
            mock_select.return_value = MagicMock(ask=MagicMock(return_value="Create a new category"))
    ...
    def test_manage_categories_view(self, tmp_db_path):
        with patch('questionary.select') as mock_select, \
            ...
            mock_select.return_value = MagicMock(ask=MagicMock(return_value="View all categories"))
    ...
    def test_manage_categories_update(self, tmp_db_path):
        with patch('questionary.select') as mock_select, \
            ...
            mock_select.return_value = MagicMock(ask=MagicMock(return_value="Update a category"))
    ...
    def test_manage_categories_delete(self, tmp_db_path):
        with patch('questionary.select') as mock_select, \
            ...
            mock_select.return_value = MagicMock(ask=MagicMock(return_value="Delete a category"))
    ...
    def test_manage_categories_assign(self, tmp_db_path):
        with patch('questionary.select') as mock_select, \
            ...
            mock_select.return_value = MagicMock(ask=MagicMock(return_value="Assign habit to category"))
    ...
    def test_manage_categories_back(self, tmp_db_path):
        with patch('questionary.select') as mock_select:
            ...
            mock_select.return_value = MagicMock(ask=MagicMock(return_value="Back to Main Menu"))
