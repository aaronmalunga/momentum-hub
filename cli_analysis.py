...
def analyze_habits(db_name: str):
    ...
    analysis_actions = {
        ...
        "List all habits with the same periodicity": lambda: analyze_by_periodicity(
            db_name
        ),
        "See the longest streak of all habits": lambda: analyze_longest_streak_all(
            db_name
        ),
        "See the longest streak of a specific habit": lambda: analyze_longest_streak_one(
            db_name
        ),
        "Show streak history (Calendar View)": lambda: analyze_streak_history_grid(
            db_name
        ),
...
def analyze_by_periodicity(db_name: str):
    ...
    if (
        periodicity_choice is None
        or periodicity_choice == "Cancel"
        or not periodicity_choice
    ):
        show_colored_message(
    ...
    if not filtered_habits:
        ...
        for habit in filtered_habits:
            ...
            current_streak_col = (
                f"{Fore.GREEN}{habit.streak}{Style.RESET_ALL}"
                if habit.streak > 0
                else f"{Fore.RED}{habit.streak}{Style.RESET_ALL}"
            )
            completion_rate = analysis.calculate_completion_rate_for_habit(
                habit.id, db_name
            )
            completion_rate_col = (
                f"{Fore.GREEN}{completion_rate*100:.0f}%{Style.RESET_ALL}"
                if completion_rate >= 0.8
                else (
                    f"{Fore.YELLOW}{completion_rate*100:.0f}%{Style.RESET_ALL}"
...
def analyze_streak_history_grid(db_name: str):
    ...
    if habit.frequency == "daily":
        ...
        if total_completions > 0:
            ...
            show_colored_message(
                f"Completion rate this month: {completion_rate*100:.1f}% ({month_completions}/{days_in_month} days)",
                color=Fore.MAGENTA,
            )
        ...
        if weeks_to_show > 0:
            ...
            show_colored_message(
                f"Completion rate last {weeks_to_show} weeks: {weekly_completion_rate*100:.1f}% ({completed_weeks}/{weeks_to_show} weeks)",
                color=Fore.MAGENTA,
            )
    else:
        show_colored_message(
